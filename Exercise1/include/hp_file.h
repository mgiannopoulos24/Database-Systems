...
#define HP_ERROR -1                 // Κωδικός σφάλματος
#define HP_OK 0                     // Κωδικός επιτυχίας

/* Η δομή HP_info κρατάει μεταδεδομένα που σχετίζονται με το αρχείο σωρού */
typedef struct {
    int fileDesc;               // Αναγνωριστικό αρχείου
    char fileName[HP_MAX_FILENAME_LENGTH]; // Όνομα αρχείου
    int lastBlockId;            // Αναγνωριστικό τελευταίου block
    int recordsPerBlock;        // Αριθμός εγγραφών ανά block
} HP_info;

/* Η δομή HP_block_info κρατάει πληροφορίες για τα blocks του αρχείου σωρού */
typedef struct {
    int numRecords; // Αριθμός εγγραφών στο block
    int nextBlock;  // Αναγνωριστικό επόμενου block (-1 αν δεν υπάρχει)
} HP_block_info;
